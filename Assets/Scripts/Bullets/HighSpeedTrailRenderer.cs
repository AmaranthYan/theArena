...
public class HighSpeedTrailRenderer : MonoBehaviour 
{
	...
	//在Trail中插入点，解决高速物理运算遗漏点的问题
	public void insertPoint(Vector3 position, Quaternion rotation)
	{
		for(int i = pointCnt; i > 0; i--)
			points[i] = points[i-1];
		points[0] = new Point(position, rotation);
		pointCnt++;
	}
